    ...
    init(pokemonDetail: PokemonDetail, pokemonSpecies: PokemonSpecies) {
        ...
        if pokemonSpecies.gender_rate == -1 {
            ...
            let femalePercentage =
                (Float(pokemonSpecies.gender_rate) / 8.0) * 100.0
            let malePercentage = 100.0 - femalePercentage
            let malePercentageString =
                "\(String(format: "%.1f", malePercentage))%"
            let femalePercentageString =
                "\(String(format: "%.1f", femalePercentage))%"
            gender = Gender(
                malePercentage: malePercentageString,
                femalePercentage: femalePercentageString)
        }
            ...
            for stat in pokemonStats {
                stats.append(
                    Stathistic(
                        ...
                        valueFloat: Float(stat.base_stat) / 100.0))
                total += stat.base_stat
            }
            stats.append(
                Stathistic(
                    name: "total", value: total,
                    valueFloat: Float(total) / Float(stats.count * 100)))
        }

        self.color =
            UIConstants.Pokedex.pokedexColorMap[pokemonSpecies.color.name]
